...
public class Role implements Serializable {
    ...
    @Column(columnName = "role_id", isPrimaryKey = true, isNull = false, jdbcType = JdbcType.BIGINT, comment = "主键ID", length = 32)
    private Long roleId;
    ...
    @Column(columnName = "creator", jdbcType = JdbcType.VARCHAR, comment = "创建者", length = 32)
    private Long creator;

    @Column(columnName = "create_date", jdbcType = JdbcType.DATE)
    private Date createDate;

    @Column(columnName = "modifier", length = 16, jdbcType = JdbcType.BIGINT, comment = "修改者")
